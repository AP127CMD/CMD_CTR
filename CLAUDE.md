...
Auto-ranks AP-127 students by progress (most-behind first) and proposes the best open slot per student. Current state: **r39**.
- **Rank source**: fetches `cache.json` from `ap127-db001.pages.dev/` on mount (with raw GitHub fallback); cached in `localStorage` for 6h; manual **⟳ SYNC SP INFO** button (blue)
...
- **Name mapping**: `asfShortName("Akaravit Khwanngam")` → `"Akaravit K."` — joins with flight `student` field (case-insensitive)
...
- **SP eligibility guards** (all checked before slot search, in order):
  1. SP on leave (`leavesMap` match) → `slots=[]`, blue **ON LEAVE** badge
  2. SP already has a real flight today (`dateFlights` match, non-cancelled) → `slots=[]`, green **SCHEDULED** badge
  3. Both also skip in `bulkReserve` and `baselineSlotsByStudent`
- **FI eligibility**: FIs on leave are excluded from `candidates.candFIs`; shown with blue **LEAVE** chip in FI FILTER dropdown
- **A/C eligibility**: maintenance tails excluded from `candidates.candTails`; shown with red **GND** chip in A/C rows
- **Per-student slot search**: per-SP `asfBuildBusyMap(augmentedFlights, gap)` — each SP's buffer applied independently; cascading reservation via `augmentedFlights` (activated virtual flights block other SPs); FI availability covers cross-batch flights (FI appearing as student in FAM FI / PPC flights correctly blocked)
- **Student card** (compact single-line summary row): rank pill (22px) · name · nick · `│` · `NEXT xx` · `IDLE Nd` · spacer · status badges · slot count · `▸` chevron; `flexWrap:'wrap'` reflows to second line on narrow screens; summary row is a `<div>` (not button) to avoid button-in-button nesting
- **Status badges** in summary row: `★ HH:MM–HH:MM` (reserved, highlight) · RELEASE · PROPOSE ▸ · ON LEAVE (blue) · SCHEDULED (green) · slot count chip · `−N BLOCKED` (amber) · `BLOCKED BY OTHERS` (red)
- **Per-SP filter row** (always visible, below summary): FI / SE TYPE / DURATION / BUFFER dropdowns
- **`AsfSlotBtn`**: slot card with time, combos, RESERVE ▲ (opens `AsfAcPickerModal` at board level) / RELEASE ▼; A/C tail chips directly reserve that FI+tail on click; hover synced with timeline + mini-timeline
- **`AsfAcPickerModal`**: lifted to board level (not inside `AsfSlotBtn`) — prevents hover flicker; `anyModalOpen` flag dims content with `pointerEvents:'none'` + `opacity:0.45` while any modal is open
...
- **TYPE / FI FILTER**: `AsfMultiCheck` dropdown — `null`=ALL (all boxes checked), `[]`=NONE (all unchecked), `string[]`=explicit subset; **ALL** and **NONE** buttons in dropdown header; backdrop div for outside-click; trigger button turns red when NONE active; LEAVE/GND badges on items
- **Main timeline** (`AsfTimeline`): 2-column grid (`LABEL_W 1fr`) — header and rows share the same column so ruler aligns perfectly with bars; FI tab + A/C tab (buttons inside label cell); rows filtered by active TYPE/FI filter (FIs with reservations always shown); grey = scheduled flights (all batches, all instructors); green = available slots; highlight = reserved; hover synced; on-leave FI rows dimmed + LEAVE badge; maintenance A/C rows dimmed + GND badge
- **`dynHourEnd`**: timeline end hour = `max(18, ceil(last_flight_end_min / 60))` — passed to `AsfTimeline` and `AsfMiniTimeline`
...
- **Persistence**: `ASF_SETTINGS_LS_KEY = 'ap127-asf-settings-v1'` saves 11 fields (`acTypeFilter, fiFilter, fiMatchSp, windowFrom, windowTo, rwyEnabled, rwyFrom, rwyTo, sortMode, topN, onlyOpen`) on every change; `asfLoadSettings()` reads on mount. Reservations and per-SP overrides are date-scoped — NOT persisted. `↺ RESET` restores `ASF_DEFAULTS` and clears the key. `ASF_DEFAULTS`: `acTypeFilter:null, fiFilter:null` (null = ALL)
- **Bulk operations**: `⚡ AUTO RESERVE` (shown when 0 reservations) walks `finalRecords` in rank order, skipping on-leave and already-scheduled SPs, picks earliest slot + first FI/tail pair, cascades. `📋 EXPORT ALL` (shown when ≥1 reservation) copies dispatcher-ready numbered proposal text to clipboard with `✓ COPIED` feedback
- **Cascade feedback**: `baselineSlotsByStudent` computes slots using `dateFlights` only (no reservations); each SP card shows amber `−N BLOCKED` chip (reduced) or red `BLOCKED BY OTHERS` chip (eliminated); suppressed for SPs with their own reservation
- **WITH SLOTS ONLY**: hides SPs with 0 slots (on-leave, already-scheduled, blocked) AND SPs that already have a reservation (focus on pending work)
- **SP-FI MATCHED toggle** (default ON): when ON, per-SP search pins `spCandFIs` to the SP's preferred FI; when OFF, all `candidates.candFIs` are tried
- **Controls**: DATE · TYPE · SHOW (top N) · FI FILTER · FROM/TO · RWY CLOSE · ⚡ AUTO RESERVE / 📋 EXPORT ALL · RELEASE ALL · WITH SLOTS ONLY · SP-FI MATCHED · EXPAND ALL · COLLAPSE · ↺ RESET
...
| 38 | **AUTO SLOT FINDER — exclude SPs already scheduled that day**: Any SP who has a non-cancelled flight in `dateFlights` for the selected date gets `slots=[]` everywhere — `slotsByStudent`, `baselineSlotsByStudent`, and `bulkReserve` all skip them with a `spHasFlight` guard (`asfShortName` case-insensitive match on `f.student`). `finalRecords` carries a new `hasFlight: bool` flag; `AsfStudentCard` shows a green **SCHEDULED** chip in the header summary row (identical styling to ON LEAVE but green). The slot count badge reads "0 SLOTS" as usual, and "WITH SLOTS ONLY" hides them (they have 0 slots). `dateFlights` added to `slotsByStudent` and `finalRecords` dep arrays. Cache token → `r38`. |
| 39 | **AUTO SLOT FINDER — timeline filter respect, WITH SLOTS ONLY hides reserved, compact SP row**: (1) **Timeline respects TYPE/FI filter** — `allFIsForTimeline` now filters by `fiFilter` (if set) and `acTypeFilter` (via `fiQuals`); FIs with activated reservations always stay visible regardless of filter. `allTailsForTimeline` now also filters `dateFlights` tails by `acTypeFilter` via `tailTypeMap` (previously only RESOURCES tails were filtered); activated reservation tails always visible. (2) **WITH SLOTS ONLY hides reserved SPs** — `finalRecords` filter (`onlyOpen`) now also excludes SPs that already have a reservation (`activatedSlots[spKey]`), so the "open" list focuses purely on pending SPs; `activatedSlots` added to dep array. (3) **Compact single-line SP card summary row** — removed the two `flexDirection:'column'` sub-divs (name+nick stacked; NEXT+IDLE stacked); all fields are now flat flex items on one row: rank pill (22px) · name · nick · `│` separator · `NEXT xx` · `IDLE Nd` · spacer · badges · chevron. `flexWrap:'wrap'` still allows them to reflow to a second line on narrow screens. Vertical padding reduced from 8px→6px; filter row left-indent tightened to match new pill width. Cache token → `r39`. |
